...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function removeFromListBox(boxId, text) {
    ...
    for (let i = 0; i < list.length; i++) if (list.options[i].value === text) list.remove(i--);
  }

  // existing state
  for (let i = 0; i < localStorage.length; i++) {
    const domain = localStorage.key(i);
    if (Number(localStorage.getItem(domain)) === 1) appendToListBox('excludedDomainsBox', domain);
  }
  ...
  importButton.addEventListener('click', function() {
    ...
    request.onload = function() {
      if (this.status >= 200 && this.status < 400) {
        ...
        for (const domain in data) localStorage[domain] = data[domain];
        location.reload();
      } else {
  ...
  buildin_checkbox.addEventListener('click', () => {
    console.log(buildin_checkbox.checked);
  });
